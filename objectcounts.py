...
:store_file: Is the full path to the destination XSLX file, created if not present

The output is a new or updated spreadsheet that contains at least two worksheets.

**Summary** contains the totals for different object types (DATA, METADATA, RESOURCE)
for the categories listed above. Column names are::
  ...
  J pubo_Data      Number of publicly accessible, not obsoleted data objects
  K pubo_Metadata  Number of publicly accessible, not obsoleted metadata objects
  L pubo_Resource  Number of publicly accessible, not obsoleted resource objecs
  M pubo_Total     Total number of publicly accessible, not obsoleted objects


A sheet named **YYYY-MM-DD** is optionally created when run with the -d or --detail
option. That sheet contains a breakdown of counts for each object format.

..Note:: 

  This script is currently running on monitor.dataone.org just past midnight ET, 
  with detail sheets generated on Fridays. The folder containing the spreadsheet can 
  be accessed with the BTSync read only key: BEUZJ2KNWPOKZ6ED6G4HB45AT55YW4JZ3
...
def getObjectFormats(base_url):
  ...
  for format in formats.objectFormat:
    res[format.formatId] = [format.formatName, format.formatType, 0, 0, 0]
  return res
...
def countObjects(base_url, formats):
  ...
  for formatId in formats.keys():
    ...
    formats[formatId][2] = res.total
    q = "formatId:\"{0:s}\"".format(escapeQueryTerm(formatId))
    formats[formatId][3] = getHitCount(oldclient, q)
    q = "formatId:\"{0:s}\" AND -obsoletedBy:[* TO *]".format(escapeQueryTerm(formatId))
    formats[formatId][4] = getHitCount(oldclient, q)
...
def populateWorksheet(ws, formats, label=''):
  ...
  counts = getTotalsByType(formats)
  crow = 3
  ...
  for formatId in formats.keys():
    ...
    ws.cell(rc(crow,2)).value = formats[formatId][1]
    ws.cell(rc(crow,3)).value = formats[formatId][2]
    ws.cell(rc(crow,4)).value = formats[formatId][3]
    ws.cell(rc(crow,5)).value = formats[formatId][4]
    crow += 1
...
if __name__ == "__main__": 
  ...
  formats = getObjectCounts( base_url )
  summarysheet = None
  try:
